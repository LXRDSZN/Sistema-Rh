...
const aplicarFiltros = () => {
  const area = normalizarTexto(selectedArea.value)
  const term = normalizarTexto(searchTerm.value).trim()

  const filtrados = originalVisits.filter(v => {
    const areaVisita = normalizarTexto(v.areaVisitada)

    // Filtro por área (incluye "todas")
    const cumpleArea =
      area === '' ||
      area === 'todas' ||
      areaVisita === area

    // Filtro por texto (visitante, persona visitada, empresa)
    const texto = normalizarTexto(
      `${v.visitante} ${v.personaVisitada} ${v.empresaPertenece}`
    )
